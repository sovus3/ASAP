						...
						<div class="col-lg-12 grid-margin stretch-card">
							<div class="card">
								...
								<div class="form-group"
										...
										<input type="button" onClick="fn_upload(${curPage})" class="btn btn-primary" value="상품 올리기" >
										</div>
										</div>
												...
												<tr>
													...
													<c:if test="${!empty list}">
														...
														<tr>
															...
															<td><img src="/resources/upload/product/${product.productSeq}.png"></td>
															<td>${product.productName}</td>
															<td>₩<fmt:formatNumber type="number" maxFractionDigits="3" value="${product.productPrice}" /></td>
															<td>${product.productQuantity}</td>
															<td>${product.status}</td>
															<td><c:set var="originalDate" value="${product.productStartDate}" />
																<c:set var="dateParts" value="${fn:split(originalDate, ' ')}" />
																<c:set var="dateOnly" value="${dateParts[0]}" />
																${dateOnly}</td>
															<td><c:set var="originalDate1" value="${product.productEndDate}" />
																<c:set var="dateParts1" value="${fn:split(originalDate1, ' ')}" />
																<c:set var="dateOnly1" value="${dateParts1[0]}" />
																<span>${dateOnly1}</span></td>
															<td ><label class="badge badge-danger" style="cursor:pointer;" onclick="fn_update(${product.productSeq})">수정</label>
																&nbsp; <label class="badge badge-warning" style="cursor:pointer;" onclick="fn_delete(${product.productSeq})">삭제</label></td>									
														</tr>
