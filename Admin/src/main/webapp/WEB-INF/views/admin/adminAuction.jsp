...
.input-group-prepend .mdi-magnify {
	font-size: 15px;
}
.total {
...
$(document).ready(function(){
	...
	$("#btnSearch").on("click", function(){
		...
		document.bbsForm.action = "/admin/adminAuction";
...
function aucInsert(vrSeqValue){
	
	$.ajax({
		type: "POST",
		url: "/admin/aucInsertProc",
		data: {
			vrSeq: vrSeqValue,
			curPage: $("#curPage").val()
		},
		...
		success: function(result) {
			...
			{
				fn_list($("#curPage").val());
				Swal.fire('경매 업로드 성공', '', 'success');
			} 
			else if(result.code == 0)
			{
				fn_list($("#curPage").val());
				Swal.fire('경매 업로드 실패', '', 'error');
			}
			else if(result.code == 401)
			{
				Swal.fire({
					...
					location.href = "/admin/adminAuction";
				})
			}
...
<body>
								...
								<div class="card-body">
									...
									<div class="form-group"
											...
											<select  id="month" name="month"
												...
												<div id="btnSearch"
													class="input-group-prepend bg-transparent" style="width: 100px; height: 41px; margin-left: 5px;">
													<i
														class="input-group-text bg-transparent border-0 mdi mdi-magnify" style="cursor:pointer; border-radius:20%"></i>
												</div>
											</div>
											...
											<c:if test="${!empty list}">
												<c:forEach var="voteUpload" items="${list}" varStatus="status">
													<tr>
														...
														<td>${voteUpload.vrSeq}</td>
														<td>${voteUpload.userId}</td>
														<td>
															<div>
																<c:choose>
														...
														<td>${voteUpload.vrTitle}</td>
														<td><fmt:formatNumber type="number" maxFractionDigits="3" value="${voteUpload.vrTotalCnt}"/></td>
														<td><c:set var="originalDate1" value="${voteUpload.vrStartDate}" />
															<c:set var="year1" value="${fn:substring(originalDate1, 0, 4)}" />
															<c:set var="month1" value="${fn:substring(originalDate1, 4, 6)}" />
															<c:set var="day1" value="${fn:substring(originalDate1, 6, 8)}" />
															${year1}.${month1}.${day1}</td>
														<td>
														<c:set var="originalDate" value="${voteUpload.vrEndDate}" />
															<c:set var="year" value="${fn:substring(originalDate, 0, 4)}" />
															<c:set var="month" value="${fn:substring(originalDate, 4, 6)}" />
															<c:set var="day" value="${fn:substring(originalDate, 6, 8)}" />
															${year}.${month}.${day}</td>
														<td>
															<div>
																<c:choose>
																	<c:when test="${voteUpload.aucSeq == 0}">
																		<input type="button" id="aucInsert" name="aucInsert" class="btn btn-primary"
																			onclick="aucInsert(${voteUpload.vrSeq})" value="업로드">
																	</c:when>
																	<c:otherwise>
																		<span>업로드 완료</span>
																	</c:otherwise>
	...
	<form name="bbsForm" id="bbsForm" method="post">
		...
		<input type="hidden" id="gnbAdmId" value="${gnbAdmId }"/>
	</form>
