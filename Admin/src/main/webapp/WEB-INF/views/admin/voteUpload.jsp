				...
				<div class="content-wrapper">
					...
					<div class="row">
						<div class="col-lg-12 grid-margin stretch-card">
							<div class="card">
								<div class="card-body">
									<h4 class="card-title">투표 내역 및 투표자 조회</h4>
											...
											<thead>
												...
												<c:forEach var="list" items="${list}" varStatus="status">
														<tr>
															<td>${list.vrSeq}</td>
															<td >${list.userId}</td>
															<td style="white-space: normal; word-wrap: break-word;">${list.vrTitle}</td>
															<td >${list.categoryName}</td>
															<td ><fmt:formatNumber type="number" maxFractionDigits="3" value="${list.vrTotalCnt}"/></td>
															<td ><fmt:formatNumber type="number" maxFractionDigits="3" value="${list.vrStartPrice}"/></td>
															<td >${list.regDate}</td>	
															<td>
																			<c:choose>
																		...
																		<td>
																			<c:choose>
																				<c:when test="${list.status == 'P'}">
																					<input type="button" class="btn btn-primary btn-xs" onclick="buttonClick(${list.vrSeq})" value="업로드">
																				</c:when>
																				<c:otherwise>승인완료
																				</c:otherwise>
																			</c:choose>
																		...
																		<td>
																			<div class="voteList">
																				<a href="javascript:void(0)" style="cursor:pointer;" onclick="fn_voteList(${list.vrSeq})"> 
																					투표자 확인
																				</a>
																			</div>
																		</td>														
