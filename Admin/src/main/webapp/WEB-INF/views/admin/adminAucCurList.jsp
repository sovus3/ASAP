...
function fn_list(curPage){
    ...
    document.bbsForm.searchType.value = $("#searchType").val();
	document.bbsForm.searchValue.value = $("#searchValue").val();
	document.bbsForm.searchDate.value = $("#searchDate").val();
	document.bbsForm.action="/admin/adminAucCurList";
	document.bbsForm.submit();
}
...
function setDateBox(){
	...
	{	
		...
		for(var i = 1; i <= 12; i++)
		{
			if(i.toString().length == 1) //i.toString().length == 1
			{
				i = "0" + i;
...
<body>
											...
											<c:if test="${!empty list}">
												<c:forEach var="aucCur" items="${list}" varStatus="status">
													<tr>
														...
														<td>${aucCur.userId}</td>
														<td><fmt:formatNumber type="number" maxFractionDigits="3" value="${aucCur.aucCurPrice}"/></td>
	...
	<form name="bbsForm" id="bbsForm" method="post">
		<input type="hidden" name="vrSeq" id="vrSeq" value="" /> <input
			type="hidden" id="curPage" name="curPage" value="${curPage}" /> <input
			type="hidden" id="searchDate" name="searchDate" value="${searchDate}" /> <input
			type="hidden" id="yearValue" name="yearValue" value="${yearValue}" />
		<!-- modelMapd의 변수 -->
