...
function setOrderInfo(){
	...
	let deliveryFee = 2500;
	let count = $("#count").val();   
	let totalQuan = parseInt($("#totalQuantity").val()); 
	...
	{
		itemName = $("#itemName").val() + ' 외 ' + (count -1) + '개';   
	}
	console.log("itemName:", itemName);
					   ...
					   type: "POST",
						...
						success:function(response)
						{
							if(response.code == 0)
							{
								$.ajax({
							         type:"POST",
							         url:"/product/orderProc",
							         data:{
							             userId:$("#userId2").val(),
							             orderTotalPrice:finalTotalPrice,
							             orderTotalQuantity:totalQuan,
         ...
         <div class="col-lg-4">
               ...
               <div class="card-body price">
                  ...
                  <c:if test="${!empty list}">
                     ...
                     <c:forEach var="cart" items="${list}" varStatus="status">
                        <c:set var="cartQuantity" value="${cart.cartQuantity }" />
                        <c:set var="price" value="${cart.productPrice * cart.cartQuantity}" />
                        <c:set var="count" value="${ count + 1 }" />
                        <div class="d-flex justify-content-between">

                           <p class="mt-0 mb-0 " align="right"
                              style="margin: 0px 0px 0px 0px; line-height: 120%;">${cart.productName}</p>
                           <p class="mt-0 mb-0 " align="right"
                              style="margin: 0px 0px 0px 0px; line-height: 120%;">${cart.cartQuantity}개</p>
                           <p class="mt-0 mb-0 " align="right"
                              style="margin: 0px 0px 0px 0px; line-height: 120%;">
                              &#8361;
                              <fmt:formatNumber type="number" maxFractionDigits="3"
                                 value="${cart.productPrice * cart.cartQuantity}" />
                           </p>
                        </div>
        				&nbsp;
        				<c:set var="totalPrice" value="${totalPrice + price}" />
                        <c:set var="totalQuantity"
                           value="${totalQuantity + cartQuantity}" />
                        <input type="hidden" id="userId2" value="${cart.userId}">
                        <input type="hidden" id="itemName" value="${cart.productName}">
                        <input type="hidden" id="itemCode" value="${cart.productSeq }">
                     </c:forEach>
                  </c:if>
                  <input type="hidden" id="count" value="${count }">
                  <input type="hidden" id="totalQuantity" value="${totalQuantity}">
                  <input type="hidden" id="totalPrice" class="totalPrice_input"
                     value="${totalPrice}">
            ...
            <div class="card mb-5" style="border-color: #3D4F74;">
               ...
               <div class="card-footer bg-transparent"
                  style="border-color: #3D4F74;">
                  <button
                     class="btn btn-lg btn-block font-weight-bold my-3 py-3 pay"
                     style="background-color: #3D4F74; color: #ffffff;"
                     onclick="setOrderInfo()">결제하기</button>
               </div>
            </div>
         </div>
      </div>
